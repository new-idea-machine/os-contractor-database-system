...
import "./SearchProjectManagers.css";
...
export default function Search() {
  ...
  useEffect(() => {
    const contractorSkillsList = () => {
      ...
      for (const contractor of contractorList) {
        ...
        // Filter contractors by "Project Manager" qualification
        const isProjectManager = contractor.qualification === "Project Manager";
    
        const percentMatching = selectedOptions.length
          ? Math.round((numMatchingSkills / selectedOptions.length) * 100)
          : 100;
    
        if (numMatchingSkills > 0 && isMatchingLocation && isProjectManager) {
  ...
  return (
    <div>
      <Navigation />
        <h1 style={{textAlign: "center", backgroundColor: "#B2B2B2", padding: "3px"}}>Project Managers</h1>
