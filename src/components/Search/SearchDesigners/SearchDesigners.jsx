...
export default function Search() {
  ...
  useEffect(() => {
    const contractorSkillsList = () => {
      ...
      for (const contractor of contractorList) {
        ...
        // Filter contractors by "Designer" qualification
        const isDesigner = contractor.qualification === "Designer";
    
        const percentMatching = selectedOptions.length
          ? Math.round((numMatchingSkills / selectedOptions.length) * 100)
          : 100;
    
        if (numMatchingSkills > 0 && isMatchingLocation && isDesigner) {
  ...
  return (
    <div>
      <Navigation />
        <h1 style={{textAlign: "center", backgroundColor: "#B2B2B2", padding: "3px"}}>Designers</h1>
